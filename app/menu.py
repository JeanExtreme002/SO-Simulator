...
class MenuWindow(Tk):
    ...
    """
    def __init__(self, title: str = "Window", size: tuple[int] = (350, 150)):
        super().__init__()
        self.__size = size
    ...
    def __finish_config(self):
        ...
        """
        self.__quantum = self.__entry.get()

        # Deve haver um quantum para os algoritmos RR e EDF.
    ...
    def __set_algorithm(self, index: int):
        ...
        if index in [0, 1]: self.__entry.delete(0, "end")
    ...
    def build(self):
        ...
        self.__label_2 = Label(self.__frame_2, text = "Selecione o quantum do processador (inteiro):", bg = "white")
        self.__label_2.pack(side = "left")

        self.__entry = Entry(self.__frame_2, width = 4)

        self.__entry_reg = self.register(self.__validate_entry)
        self.__entry.config(validate="key", validatecommand=(self.__entry_reg, "%P"))

        self.__entry.pack(side = "left", expand = True, fill = "x")
    ...
    def get_config(self):
        ...
        """
        return self.__algorithm(), self.__quantum
